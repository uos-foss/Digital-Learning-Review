...
| **8. Tabs & Report Summary** | Accessibility Report and Module Checks and Readiness as two tabs (Module Checks first); above both, a collapsed Report Summary (whether a Digital Learning Advisor has checked the module, what's still to do, and each source's refresh date) and the advisor's comment, when there is one. | `active_row` (`Ally Last Checked`, `Readiness Snapshot`, `Leganto Snapshot`), Ally profile, findings, `audit_responses.comments` | `st.tabs()` in `view_module_report()`; summary rendered by `_render_report_summary()` from `_summary_points()`; comment by `_render_advisor_comment()`. |
...
* **Purpose**: Shows, per required Blackboard template section, whether the module lead's own content is visible to students, which is the Module Report Card's read of the faculty Template Alignment Report. Rendered by `_render_template_sections()`, one card per section via `_render_section_card()`.
...
* **Data Origin**:
  ...
  - Section labels come from `processing.TEMPLATE_SECTIONS`; the nesting and its order come from `processing.TEMPLATE_SECTION_TREE` (presentation-only, since readiness logic elsewhere still uses `LEAD_OWNED_SECTIONS`, which is unchanged). Each section's Blackboard content-type icon comes from `views/module_report.py`'s `SECTION_CONTENT_TYPE`/`CONTENT_TYPE_ICONS`.
...
### 8. Tab layout and Report Summary
* **Purpose**: Since v1.21, Accessibility Report and Module Checks and Readiness are separate tabs (`st.tabs`) rather than side-by-side columns, with Module Checks and Readiness first, since it's the actionable tab for a module lead. The metadata header, health banner, and a new data-reliability block (ingestion dates for Ally/Template Alignment/Leganto, plus a collapsed rationale expander condensed from the Help page) sit above both tabs, since they summarise across both data sources. "Additional Comments" moved into the Module Checks tab. Since 24-09-2026 the health banner, data refresh dates and rationale are one collapsed "Report Summary" expander, and the comment sits above the tabs as "Comments from your Digital Learning Advisor".
* **Code Reference**: `view_module_report()`, `_render_report_summary()`, `_summary_points()` and `_render_advisor_comment()` in [views/module_report.py](file:///c:/Users/fs1hpc/Documents/GitHub/Digital-Learning-Review/views/module_report.py).
