...
def sync_checklist_fields():
    ...
    import logging
    from data_manager import get_spreadsheet_data
    from processing import TEMPLATE_SECTIONS, SECTION_KEY_BY_AUDIT_FIELD
    ...
    try:
        ...
        # A field mapped to a Template Alignment section always keeps that
        # section's own name as its label, regardless of what either side of
        # the sync says - the Sheet is a legitimate place to reword a
        # free-standing question, but not to silently retarget which
        # Blackboard section a linked field's answer overrides. This is the
        # same lock the Admin Panel's Audit Field Manager applies on save;
        # it's needed here too since this sync can run standalone.
        def _locked_label(row):
            section_key = SECTION_KEY_BY_AUDIT_FIELD.get(row['id'])
            return TEMPLATE_SECTIONS[section_key][0] if section_key else row['label']
        df_merged['label'] = df_merged.apply(_locked_label, axis=1)
