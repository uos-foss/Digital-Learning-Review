...
def view_audit_portal(df_aut, df_spr, checklist_sums, df_assess=None):
    ...
    if not pending_spot_checks.empty:
        with st.expander(f"🎯 Your Spot-Checks ({len(pending_spot_checks)} pending)", expanded=True):
            st.caption(
                "Sampled modules queued for you to review. Open one and complete "
                "the checklist as normal - saving it records whether your answers "
                "matched what the Blackboard Template data suggested at the time "
                "it was sampled.")
            for _, sc_row in pending_spot_checks.iterrows():
                ...
                with sc_c1:
                    st.write(f"**{sc_code}** — {sc_name}  ·  sampled {sc_row['sampled_on']} "
                            f"(round {sc_row['sample_round']})")
                with sc_c2:
        ...
        with st.form("audit_portal_checklist_form"):
            ...
            if (save_draft or save_submit) and not masquerading:
                ...
                try:
                    ...
                    # If this module was queued as a spot-check for the person
                    # saving it, the first save closes it out - comparing what
                    # they just answered against the suggestion frozen at
                    # sampling time. A save by someone other than the assigned
                    # advisor (e.g. covering an absence) leaves it pending, so
                    # the agreement rate stays a measure of the assigned
                    # advisor's own judgement, not whoever happened to save it.
                    pending_sc = get_pending_spot_check(selected_code, CURRENT_ACADEMIC_YEAR)
                    if pending_sc and pending_sc.get('assigned_to') == username_upper:
