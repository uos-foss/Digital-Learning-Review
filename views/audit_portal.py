...
def view_audit_portal(df_aut, df_spr, checklist_sums, df_assess=None):
    ...
    # Main content (module selector + checklist form) on the left, the
    # spot-check queue as a small right-hand rail - same 3:1 split as
    # views/module_report.py's Blackboard Template / Actions layout. A
    # narrow, bounded-height rail keeps a faculty-wide queue from pushing the
    # form down the page, and sitting beside the form (not above it) makes
    # opening a flagged module visibly connect to where it loads.
    col_main, col_spotchecks = st.columns([3, 1])

    with col_spotchecks:
        if not pending_spot_checks.empty:
            # Same "#### <title> (N)" header + single consolidated panel
            # shape as views/module_report.py's Actions rail
            # (_render_actions_panel), but a neutral bordered container
            # instead of that panel's amber "outstanding item" styling - a
            # queued spot-check isn't a warning, just a list to work through.
            # Rows are plain list entries (label/name over an Open action,
            # divider-separated) rather than individually bordered cards.
            st.markdown(f"#### 🎯 Spot-Checks ({len(pending_spot_checks)})")
            st.caption(
                "Flagged in the school(s) you're currently working in - not "
                "just ones you flagged yourself. Open one to load it in the "
                "form on the left.")
            rows = list(pending_spot_checks.iterrows())
            with st.container(border=True):
                for i, (_, sc_row) in enumerate(rows):
                    sc_code = sc_row['module_code']
                    sc_name = module_mapping.get(sc_code, sc_code)
                    is_current = sc_code == st.session_state.get("selected_module_code")
                    row_label, row_action = st.columns([3, 1], vertical_alignment="center")
                    with row_label:
                        marker = "📍 " if is_current else ""
                        # One markdown block for code+name (instead of a
                        # separate st.caption) so the row doesn't carry two
                    ...
                    with row_action:
                        if is_current:
                            ...
                            if st.button("Open", key=f"sc_open_{sc_row['id']}", use_container_width=True):
                                ...
                                st.rerun()
                    if i < len(rows) - 1:
                        # A tight-margin rule instead of st.divider(), whose
